...
{
    Idle, Wander,GoingForTask,  PerformingTask, Dead, ChasingPlayer, BeingChased, CheckingLastPosition,KillingPlayer
}
public enum LocoMotionState
{
    Idle,Walk,Run, Interect,Jump,Crouch,Dead,Thinking
}
...
{    
    ...
    public ActivityTask CurrentActivity;
    [HideInInspector]   
    ...
    [HideInInspector]
    public float TaskPerformingTimer; 
    ...
    public Vector3 GetLocation(Transform origin, float distance, int layermask, float minimumnewdistance = 10, RandomDirectionAttribute Direction = RandomDirectionAttribute.Random)
    {
     ...
     //   int VertexIndex = UnityEngine.Random.Range(0, Manager.instance.Triangulation.vertices.Length);
        ...
        int trylimites = 200;
        int locationtrylimits = 20;
        int currenttry = 0;
       
        do
        {
            float radius = UnityEngine.Random.RandomRange(minimumnewdistance, distance);
            Vector3 randomDirection = UnityEngine.Random.insideUnitSphere * radius;
            randomDirection += origin.position;
            locationtrylimits = 20;
            bool res = false;
              ...
              if(res==false)
            {
                ...
                Debug.LogError("LOCATION GETTING FAILED. RETURNING WITH RANDOM POSITION");
                navHit.position = origin.position + new Vector3(UnityEngine.Random.RandomRange(-20, 20), 0, 0);
