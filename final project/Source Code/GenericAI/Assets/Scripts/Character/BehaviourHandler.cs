...
public class BehaviourHandler : MonoBehaviour
{
    ...
    {
        LocoMotionState[] IdleLocmotions ={ LocoMotionState.Idle, LocoMotionState.Thinking};
    ...
    public  void  SetRandomMotion(Character CharacterObj)
            {
                if(CharacterObj.Attributes.Type==CharacterType.Bot)
                    {
                        if (IsStateChangeAble(CharacterObj.Attributes.State))
                        {

                            CurrentState NewRandomState = GetRandomState(CharacterObj.Attributes.State);
                            LocoMotionState NewLocomotionState = GetRandomeLocomotion(NewRandomState);
                            CharacterObj.Attributes.AutoStateChangeDelay = GetRandomStateChangeTimer(NewRandomState);
                            CharacterObj.ConvertState(NewRandomState, NewLocomotionState);
                        }
                    }
            }
}
